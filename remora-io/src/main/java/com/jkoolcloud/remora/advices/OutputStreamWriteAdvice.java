...
public class OutputStreamWriteAdvice extends BaseTransformers implements RemoraAdvice {
	...
	public static void before(@Advice.This OutputStream thiz, //
		...
		try {
			...
			if (arguments != null && arguments.length == 1 && arguments[0] instanceof Byte) { // for a
																								// write(byte)
																								// case
				streamStats.advanceCount();
			} else if (arguments != null && arguments.length == 3) { // write(byte b[], int off, int len)
				streamStats.advanceCount((int) arguments[2]);
			} else if (arguments != null && arguments.length == 1 && arguments[0] instanceof byte[]) { // write(byte
																										// b[])
