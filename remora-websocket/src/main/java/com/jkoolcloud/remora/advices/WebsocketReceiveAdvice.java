...
public class WebsocketReceiveAdvice extends BaseTransformers implements RemoraAdvice {

	public static final String ADVICE_NAME = "WebsocketReceiveAdvice";
	public static final String[] INTERCEPTING_CLASS = { "javax.websocket.MessageHandler" };
	...
	private static ElementMatcher<? super MethodDescription> methodMatcher() {
		return (nameStartsWith("onMessage"));
	}
	...
	static AgentBuilder.Transformer.ForAdvice advice = new AgentBuilder.Transformer.ForAdvice()
			.include(WebsocketReceiveAdvice.class.getClassLoader()).include(RemoraConfig.INSTANCE.classLoader)//
			.advice(methodMatcher(), WebsocketReceiveAdvice.class.getName());
	...
	public static void before(@Advice.This Object thiz, //
		...
		try {
			ctx = prepareIntercept(WebsocketReceiveAdvice.class, thiz, method, message);
			if (!ctx.intercept) {
				return;
			}
	...
	public static void after(@Advice.This Object obj, //
		...
		try {
			ctx = prepareIntercept(WebsocketReceiveAdvice.class, obj, method, arguments);
			if (!ctx.intercept) {
				return;
			}
