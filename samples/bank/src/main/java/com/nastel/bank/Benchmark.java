...
public class Benchmark extends HttpServlet {
	...
	class MonitoredThread extends Thread {
		...
		public void run() {
			...
			for (int i = 0; i < call_count; i++) {
				if (monitored) {
					monitored_benchmark(count);
				} else {
					unmonitored_benchmark(count);
				}
			}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("    <tr>");
		out.println(
				"      <td width=50% align=right class=label style=\"border-bottom: solid 1px white\">Number of repetitions:</td>");
		out.println("      <td width=20% align=left class=value><input type=text size=8 name=count value=10000></td>");
		out.println("    </tr>");
		out.println("    <tr>");
		out.println(
				"      <td width=50% align=right class=label style=\"border-bottom: solid 1px white\">Calls per repetition:</td>");
		out.println(
				"      <td width=20% align=left class=value><input type=text size=8 name=call_count value=1000></td>");
		out.println("    </tr>");
		out.println("    <tr>");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("    <td width=50% align=left>" + unmonitored_time + " ms, Elapsed time per call: "
				+ (float) unmonitored_time / (float) call_count + " ms</td>");
		out.println("  </tr>");
		out.println("  <tr>");
		out.println("    <td width=50% align=right class=label>Monitored elapsed time:</td>");
		out.println("    <td width=50% align=left>" + monitored_time + " ms, Elapsed time per call: "
				+ (float) monitored_time / (float) call_count + " ms</td>");
		out.println("  </tr>");
		out.println("  <tr>");
		out.println("    <td width=50% align=right class=label>Difference:</td>");
		out.println("    <td width=50% align=left>" + diff + " ms, Cost per call: " + (float) diff / (float) call_count
				+ " ms</td>");
		out.println("  </tr>");
		out.println("  <tr>");
		out.println("    <td width=50% align=right class=label>Overhead:</td>");
		out.println("    <td width=50% align=left>"
				+ (monitored_time > 0 ? ((float) diff / (float) monitored_time) * 100.0 : 0.0) + "%</td>");
		out.println("  </tr>");
		out.println("</table>");
		UITemplate.bodyEnd(out);
